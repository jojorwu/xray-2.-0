...
void CVulkanHW::SelectPhysicalDevice()
{
    ...
    auto RateDevice = [](VkPhysicalDevice device) {
        ...
        for (uint32_t i = 0; i < memProperties.memoryHeapCount; i++)
        {
            ...
            {
                score += (int)(memProperties.memoryHeaps[i].size / (1024 * 1024));
            }
        }
...
void CVulkanHW::CreateTexture(uint32_t width, uint32_t height, VkFormat format, VkImageUsageFlags usage, VkImage& image, VmaAllocation& allocation, VkImageView& view)
{
    VkImageCreateInfo imageInfo = {};
    imageInfo.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
    imageInfo.imageType = VK_IMAGE_TYPE_2D;
...
void CVulkanHW::CreateSampler(VkFilter magFilter, VkFilter minFilter, VkSamplerAddressMode addressMode, VkSampler& sampler)
{
    ...
    {
        Msg("! Vulkan: Failed to create sampler!");
    }
}
...
VkCommandBuffer CVulkanHW::BeginSingleTimeCommands()
{
    ...
    allocInfo.commandPool = VulkanBackend.GetCommandPool();
...
void CVulkanHW::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipCount)
{
    ...
    {
        Msg("! Vulkan: Unsupported layout transition!");
