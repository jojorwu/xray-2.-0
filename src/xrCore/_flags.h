...
#ifndef __linux__
	IC SelfRef or (const T mask) { return _or(mask); }
	IC SelfRef or (const Self& f, const T mask) { return _or(f, mask); }
	IC SelfRef and (const T mask) { return _and(mask); }
	IC SelfRef and (const Self& f, const T mask) { return _and(f, mask); }
#endif
